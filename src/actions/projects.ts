...
async function projectData(formData: FormData) {
  ...
  return { title: value.title, slug: value.slug, shortDescription: value.shortDescription, description: value.description || null, client: value.client || null, category: value.category, technologies: value.technologies.split(",").map(v => v.trim()).filter(Boolean), coverImage: value.coverImage || null, gallery: (value.gallery || "").split("\n").map(v => v.trim()).filter(Boolean), liveUrl: value.liveUrl || null, githubUrl: value.githubUrl || null, challenges: value.challenges || null, contribution: null, memberId: value.memberId || null, completionDate: value.completionDate ? new Date(value.completionDate) : null, featured: value.featured, published: value.published };
}

export async function createProject(formData: FormData) {
  if (!(await auth())) redirect("/admin/login");
  await prisma.project.create({ data: await projectData(formData) }); revalidatePath("/"); revalidatePath("/projects"); revalidatePath("/team"); redirect("/admin/projects?success=created");
}
export async function updateProject(id: string, formData: FormData) {
  if (!(await auth())) redirect("/admin/login");
  await prisma.project.update({ where: { id }, data: await projectData(formData) }); revalidatePath("/"); revalidatePath("/projects"); revalidatePath("/team"); redirect("/admin/projects?success=updated");
}
